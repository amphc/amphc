...
def gen_proc_datacube(totalcubes, weightFunction=None):
    ...
    Zs = pydd_kernel_opt.gen_beamforming_Z(pulseDataNoPadding, d_matchFilter,
                                           steerVector1, steerVector11,
                                           numPulses, numSamples, numChannels, fftSize, totalcubes)

    return Zs

if __name__ == '__main__':
    totalcubes = int(sys.argv[1]) if len(sys.argv) > 1 else 4
    
    # weight can be loaded; set to None for now
    weightFunction = None
    
    t1 = time.time()
    Zs = gen_proc_datacube(totalcubes, weightFunction)
    t2 = time.time()
