...
def gen_SteerVector(
    ...
    arrayGridx = np.tile(arrayPosx, (25, 1)).T
    arrayGridy = np.tile(arrayPosy, (40, 1))
...
t1 = time.time()
totalcubes: int = int(sys.argv[1]) if len(sys.argv) > 1 else 2
lightSpeed: float = 0.299792458 * 1e9
centerFreq: float = 9e9
sampleRate: float = 1000e6
pulseWidth: float = 30e-6
...
Z: numpy.ndarray[numpy.complex128][2, 128, 32768] = gen_beamforming_Z(
    numSamples,
    numChannels,
    steerVector1,
    steerVector11,
